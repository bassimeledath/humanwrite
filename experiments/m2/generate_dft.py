"""Adapter-native matched generation for the prospective M2 DFT comparison."""
...
from experiments.m2.dft import FULL_BRIEF_SERIALIZER_SHA256, _render_prompt, _sample_raw_policy


GENERATION_SCHEMA = "dftr.m2.adapter_native_generation.v1"
GENERATION_STEP = "generate_dft"
SAFE_ID_RE = re.compile(r"^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$")
TOP_LEVEL_KEYS = {
    "artifact_schema", "run", "compute", "model", "checkpoint", "data",
    "sampling", "runtime", "output", "workflow",
}
...
def validate_generation_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        or run.get("arm") not in {"A0-generation", "A64-generation"}
    ...
    checkpoint = _exact(
        ...
        {"path", "manifest_sha256", "adapter_model_sha256", "arm", "method_contract_sha256"},
        "checkpoint",
    )
    if (
    ...
    data = _exact(
        config.get("data"),
        {"prompt_briefs_path", "prompt_briefs_sha256", "prompt_format", "prompt_schema_version", "prompt_serializer_sha256"},
        "data",
    )
    if (
    ...
    sampling = _exact(
        config.get("sampling"),
        {"training_seed", "sampling_seed", "seed_scope", "prompt_order", "distribution", "batch_size", "new_tokens", "max_input_tokens", "decode"},
        "sampling",
    )
    if sampling != {
        "training_seed": 11,
    ...
    runtime = _exact(
        config.get("runtime"),
        {"torch_version", "transformers_version", "peft_version", "deterministic_algorithms", "cublas_workspace_config"},
        "runtime",
    )
    if (
    ...
    workflow = _exact(
        config.get("workflow"),
        {"protocol_version", "step", "generation_contract_sha256", "decoding_policy_sha256"},
        "workflow",
    )
    if (
        workflow.get("protocol_version") != GENERATION_SCHEMA
        or workflow.get("step") != GENERATION_STEP
        or workflow.get("generation_contract_sha256") != canonical_hash(generation_contract_payload(config))
        or workflow.get("decoding_policy_sha256") != canonical_hash(decoding_policy_payload(config))
    ):
...
def _verify_inputs(config: dict[str, Any]) -> tuple[Path, list[dict[str, Any]]]:
    ...
    if (
        ...
        or file_sha256(checkpoint_dir / "adapter_model.safetensors") != checkpoint["adapter_model_sha256"]
    ):
    ...
    rows = sorted(rows, key=lambda row: str(row.get("prompt_id") or row.get("fingerprint") or ""))
    prompt_ids = [str(row.get("prompt_id") or row.get("fingerprint") or "") for row in rows]
    if len(rows) != 64 or any(not item for item in prompt_ids) or len(set(prompt_ids)) != 64:
        raise GenerationConfigError("generation requires exactly 64 unique prompt IDs")
    return checkpoint_dir, rows


def run_generate_dft(config: dict[str, Any], run_id: str) -> dict[str, Any]:
    import torch
    import peft
    import transformers
    ...
    if not output_dir.is_absolute() or output_dir.exists() and any(output_dir.iterdir()):
        raise GenerationConfigError("generate_dft requires an empty wrapper checkpoint directory")
    ...
    base = AutoModelForCausalLM.from_pretrained(
        config["model"]["base"], revision=config["model"]["revision"],
    ...
    for start in range(0, len(rows), batch_size):
        ...
        prompts = [_render_prompt(row, {"data": config["data"]}) for row in batch]
        encoded = tokenizer(
            prompts, padding=True, truncation=True,
            max_length=config["sampling"]["max_input_tokens"], return_tensors="pt",
        )
